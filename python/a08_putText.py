...
def main():
    ...
    # ft.putText(img=image, text="두 번째 한글", org=pt2, fontHeight=30, color=(0, 0, 0), thickness=-1, line_type=cv2.LINE_AA, bottomLeftOrigin=True)

    korean_text = "조금 길지만 한글이 됩니다!!"
    font_size = 30
    # 한국어 폰트 파일 경로 (시스템 폰트 사용, 예: Windows - 'malgun.ttf', macOS - 'AppleGothic.ttf', Linux - Noto Sans)
    font_path = '/home/aa/OpenCVZoo/data/NanumPenScript-Regular.ttf'  # 경로를 실제 폰트로 변경 (없으면 다운로드: Google Noto Sans KR)
    try:
